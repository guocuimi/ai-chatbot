...
export class ChatPage {
  ...
  async isGenerationComplete() {
    const response = await this.page.waitForResponse(response => response.url().includes('/api/chat'))

    await response.finished()
  }

  async isVoteComplete() {
    const response = await this.page.waitForResponse(response => response.url().includes('/api/vote'))

    await response.finished()
  }

  async hasChatIdInUrl() {
    await expect(this.page).toHaveURL(/^http:\/\/localhost:3000\/chat\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/)
  }
  ...
  async addImageAttachment() {
    this.page.on('filechooser', async fileChooser => {
      const filePath = path.join(process.cwd(), 'public', 'images', 'mouth of the seine, monet.jpg')
  ...
  async getRecentAssistantMessage() {
    ...
    const reasoningElement = await lastMessageElement
      ...
      .isVisible()
      .then(async visible => (visible ? await lastMessageElement.getByTestId('message-reasoning').innerText() : null))
      .catch(() => null)
  ...
  async getRecentUserMessage() {
    ...
    const attachments = hasAttachments ? await lastMessageElement.getByTestId('message-attachments').all() : []
  ...
  public async isScrolledToBottom(): Promise<boolean> {
    return this.scrollContainer.evaluate(el => Math.abs(el.scrollHeight - el.scrollTop - el.clientHeight) < 1)
  }
