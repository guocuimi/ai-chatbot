...
test.describe.serial('/api/chat', () => {
  ...
  test('Ada cannot resume stream of chat that does not exist', async ({ adaContext }) => {
    const response = await adaContext.request.get(`/api/chat?chatId=${generateUUID()}`)
  ...
  test('Ada can resume chat generation', async ({ adaContext }) => {
    ...
    const [firstStatusCode, secondStatusCode] = await Promise.all([firstResponse.status(), secondResponse.status()])

    expect(firstStatusCode).toBe(200)
    expect(secondStatusCode).toBe(200)

    const [firstResponseBody, secondResponseBody] = await Promise.all([await firstResponse.body(), await secondResponse.body()])
  ...
  test('Ada cannot resume chat generation that has ended', async ({ adaContext }) => {
    ...
    const [, secondResponseContent] = await Promise.all([firstResponse.text(), secondResponse.text()])

    expect(secondResponseContent).toEqual('')
  })

  test('Babbage cannot resume a private chat generation that belongs to Ada', async ({ adaContext, babbageContext }) => {
    const chatId = generateUUID()
    ...
    const secondRequest = babbageContext.request.get(`/api/chat?chatId=${chatId}`)
  ...
  test('Babbage can resume a public chat generation that belongs to Ada', async ({ adaContext, babbageContext }) => {
    const chatId = generateUUID()
    ...
    const [firstResponseContent, secondResponseContent] = await Promise.all([firstResponse.text(), secondResponse.text()])
