...
import { chat, message, type MessageDeprecated, messageDeprecated, vote, voteDeprecated } from '../schema'
...
function getMessageRank(message: MessageDeprecated): number {
  if (message.role === 'assistant' && (message.content as MessageDeprecatedContentPart[]).some(contentPart => contentPart.type === 'tool-call')) {
    return 0
  }

  if (message.role === 'tool' && (message.content as MessageDeprecatedContentPart[]).some(contentPart => contentPart.type === 'tool-result')) {
    return 1
  }
...
function dedupeParts<T extends { type: string; [k: string]: any }>(parts: T[]): T[] {
...
function sanitizeParts<T extends { type: string; [k: string]: any }>(parts: T[]): T[] {
  ...
  for (let i = 0; i < chats.length; i += BATCH_SIZE) {
    ...
    for (const chat of chatBatch) {
      ...
      const messages = allMessages
        ...
        .sort((a, b) => {
          const differenceInTime = new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
          ...
          const projectedUISection = uiSection
            .map(message => {
              if (message.role === 'user') {
                ...
                const cleanParts = sanitizeParts(dedupeParts(message.parts || []))
