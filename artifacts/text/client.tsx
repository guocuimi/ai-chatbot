    ...
    if (streamPart.type === 'text-delta') {
      setArtifact(draftArtifact => {
        return {
          ...
          isVisible:
            draftArtifact.status === 'streaming' && draftArtifact.content.length > 400 && draftArtifact.content.length < 450 ? true : draftArtifact.isVisible,
          status: 'streaming'
        }
      })
    }
    ...
    return (
      <>
        <div className="flex flex-row px-4 py-8 md:p-20">
          ...
          {metadata && metadata.suggestions && metadata.suggestions.length > 0 ? <div className="h-dvh w-12 shrink-0 md:hidden" /> : null}
        </div>
      </>
    )
  },
