    ...
    <>
      ...
      >
        <div className="bg-muted sticky top-0 z-50 flex h-fit w-full flex-row items-center justify-between border-b border-zinc-200 px-2 py-1 dark:border-zinc-700">
          ...
          <Button variant="ghost" className="size-fit p-1 hover:bg-zinc-200 dark:hover:bg-zinc-700" size="icon" onClick={() => setConsoleOutputs([])}>
            <CrossSmallIcon />
          </Button>
        </div>
                  ...
                  {consoleOutput.contents.map((content, index) =>
                    content.type === 'image' ? (
                      <picture key={`${consoleOutput.id}-${index}`}>
                        <img src={content.value} alt="output" className="max-w-(--breakpoint-toast-mobile) w-full rounded-md" />
                      </picture>
                    ) : (
