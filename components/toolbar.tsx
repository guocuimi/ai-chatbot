...
const Tool = ({ description, icon, selectedTool, setSelectedTool, isToolbarVisible, setIsToolbarVisible, isAnimating, append, onClick }: ToolProps) => {
    ...
    <div className="relative flex flex-col items-center justify-end">
      ...
      <TooltipProvider>
        <Tooltip open={!isAnimating}>
          ...
          <TooltipContent side="left" sideOffset={16} className="bg-foreground text-background rounded-2xl p-3 px-4 text-sm">
            {LEVELS[currentLevel]}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
...
export const Tools = ({
  ...
  return (
    <motion.div className="flex flex-col gap-1.5" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
      <AnimatePresence>
        {isToolbarVisible &&
    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        {status === 'streaming' ? (
          ...
          <ReadingLevelSelector key="reading-level-selector" append={append} setSelectedTool={setSelectedTool} isAnimating={isAnimating} />
        ) : (
          <Tools
            key="tools"
            append={append}
