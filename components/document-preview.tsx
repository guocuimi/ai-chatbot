...
export function DocumentPreview({ isReadonly, result, args }: DocumentPreviewProps) {
  ...
  const { data: documents, isLoading: isDocumentsFetching } = useSWR<Array<Document>>(result ? `/api/document?id=${result.id}` : null, fetcher)
  ...
  if (artifact.isVisible) {
    if (result) {
      return <DocumentToolResult type="create" result={{ id: result.id, title: result.title, kind: result.kind }} isReadonly={isReadonly} />
    }
...
const PureHitboxLayer = ({
  ...
  return (
    <div className="absolute left-0 top-0 z-10 size-full rounded-xl" ref={hitboxRef} onClick={handleClick} role="presentation" aria-hidden="true">
...
const PureDocumentHeader = ({ title, kind, isStreaming }: { title: string; kind: ArtifactKind; isStreaming: boolean }) => (
...
const DocumentContent = ({ document }: { document: Document }) => {
  ...
  const containerClassName = cn('h-[257px] overflow-y-scroll border rounded-b-2xl dark:bg-muted border-t-0 dark:border-zinc-700', {
