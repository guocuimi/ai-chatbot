          ...
          <div className="flex flex-col">
            {[44, 32, 28, 64, 52].map(item => (
              <div key={item} className="flex h-8 items-center gap-2 rounded-md px-2">
                <div
                  className="max-w-(--skeleton-width) bg-sidebar-accent-foreground/10 h-4 flex-1 rounded-md"
                  style={
                    {
                      '--skeleton-width': `${item}%`
        ...
        <SidebarGroupContent>
          <SidebarMenu>
            {paginatedChatHistories &&
              (() => {
                const chatsFromHistory = paginatedChatHistories.flatMap(paginatedChatHistory => paginatedChatHistory.chats)
